...
export class PromiseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this._route.snapshot.queryParams.key == this.sessionToken) {
      this.bindCorpPromiseList(this.page, this.paginationSize);
  ...
  bindCorpPromiseList(pageNumber: number, pageSize: number) {
    this.storePage.pageNumber = pageNumber;
    this.storePage.pageSize = pageSize;
    this._http.get(this._globalService.hostPath + '/api/Promise/GetPromisedCorporate?key=' + this.sessionToken + '&page=' + pageNumber + '&pagesize=' + pageSize + '&IsPromise=' + true)
      .subscribe(
        result => {
          debugger;
          var resultData = result.json();
  ...
  submitCorporate() {
    ...
    this._http.post(this._globalService.hostPath + '/api/CorporateAction/PostStatus?Id=' + this.viewCorporate.id + '&key=' + this.sessionToken, this.submitFlwupCorpObj)
      .subscribe(result => {
        console.log(result.json());
        var resultData = result.json();
        this.bindCorpPromiseList(this.storePage.pageNumber, this.storePage.pageSize);
        $('#floatalert').fadeIn();
  ...
  currentPageBind(currPage: any) {
    ...
    this.page = currPage;
    this.bindCorpPromiseList(currPage, this.paginationSize);
    $("ul.pagination li").removeClass("active");
  }
  ...
  previousPage(prevPageNumber: number) {
    if (prevPageNumber >= 0) {
      ...
      this.pageShow = this.pageShow - 10;
      this.bindCorpPromiseList(this.pageShow + 1, this.paginationSize);
    }
  }
